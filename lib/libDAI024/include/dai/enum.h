...
\
        ...
        friend std::basic_istream<CharT, Traits>& operator >> (std::basic_istream<CharT, Traits>& is, x& y) {\
            std::string s;\
            is >> s;\
            y = x(s.c_str());\
            return is;\
...
\
        template< class CharT, class Traits > \
        friend std::basic_ostream<CharT, Traits>& operator << (std::basic_ostream<CharT, Traits>& os, const x& y) {\
            os << (const char *)y;\
            return os;\
        }\
\
