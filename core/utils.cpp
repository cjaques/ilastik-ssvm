...
string getNthDirectoryFromPath(string path, int n) {
  ...
  int len = pos1-pos2-1;
  if(pos2 < 0 || len < 0) {
    return getLastDirectoryFromPath(path);
  }
              ...
              Slice_P*& slice)
{
  ...
  printf("[utils]:2151 - Chris hack - force using 3d slice \n");
  bool useSlice3d = true; //paramSlice3d.c_str()[0] == '1'; //TODO CHris : correct something, either config file or arg parser. 
...
void loadDataAndFeatures(string imageDir, string maskDir, Config* config,
                         Slice_P*& slice, Feature*& feature, int* featureSize)
{
  slice = 0;
  feature = 0;
  string config_tmp;
...
int enumerateLabels(const char* dir, const char* ext)
{
  ...
  for(vector<string>::iterator itFilename = lImageNames.begin();
    ...
    if(!img) {
      ...
      for(int y = 0; y < img->height ; y++) {
        for(int x = 0; x < img->width ; x++) {
          uchar r = ((uchar*)(img->imageData + img->widthStep*y))[x*img->nChannels];
          uchar g = ((uchar*)(img->imageData + img->widthStep*y))[x*img->nChannels+1];
          uchar b = ((uchar*)(img->imageData + img->widthStep*y))[x*img->nChannels+2];          
          ulong classIdx = RGBToclassIdx(r, g, b);
          classIndices.insert(classIdx);
        }
      }
    }
  }

  printf("[utils] Class Indices:\n");
  for(set<ulong>::iterator it = classIndices.begin(); it != classIndices.end(); ++it) {
    printf("%ld\n", *it);
  }
 ...
 * Write of .gz files
 */
int compress_gzio(const char *src, const char* dest)
{
#ifdef NO_GZCOMPRESS
    ...
    if (!input_file) {
      printf("[utils] Error while opening %s\n", src);
      return -1;
    }
    ...
    unsigned char* buffer = new unsigned char[MAX_FILE_SIZE];
    int n = 0;

    do {
      n = fread(buffer, 1, MAX_FILE_SIZE, input_file);
      gzwrite(file, buffer, n);
    } while(n > 0);
...
void zipAndDeleteCube(const char* cubeName)
{
  ...
  string cubeName_out = cubeName_str + ".gz";

  int ret = compress_gzio(cubeName_in.c_str(), cubeName_out.c_str());
...
{
  stringstream cmd;
  cmd << "zip " << cubeName << ".zip ";
#ifdef USE_ITK
  cmd << cubeName << ".tif ";
  //cmd << "; cubeToImages -n 1 -f 10 -i " << cubeName << ".tif ";
  cmd << "; rm " << cubeName << ".tif";
#else
  cmd << cubeName << ".nfo ";
  cmd << cubeName;
  //cmd << "; cubeToImages -n 1 -f 10 -i " << cubeName;
  cmd << "; rm " << cubeName;
  cmd << "; rm " << cubeName << ".nfo";
#endif
  // printf("[utils] Running command %s\n", cmd.str().c_str());
  if(system(cmd.str().c_str()) < 0) {
    printf("[utils] Error while running command %s\n", cmd.str().c_str());
  }
}
*/
