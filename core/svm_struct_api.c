                                 ...
                                 STRUCT_LEARN_PARM *sparm)
{
  ...
  string config_tmp;
  bool rescale_loss_per_supernode = false;
  if(Config::Instance()->getParameter("rescale_loss_per_supernode", config_tmp)) {
    rescale_loss_per_supernode = config_tmp[0] == '1';
  }

  if(rescale_loss_per_supernode) {
    printf("[SVM_struct] Rescaling loss by number of supernodes\n");
    // TODO: Experimenting with trees to see if rescaling loss helps with convergence
    // normalize per number of nodes
    ulong nSupernodesForAllExamples = 0;
    for(long idx = 0; idx < nExamples; idx++) {
      nSupernodesForAllExamples += examples[idx].x.slice->getNbSupernodes();
    }
    for(int i = 0; i < nClasses; i++) {
      lossPerLabel[i] /= nSupernodesForAllExamples;
    }
  }
...
std::cerr << "image dir" << imageDir << std::endl;
  ...
  if(Config::Instance()->getParameter("featureDir", config_tmp)) {
    stringstream sout;
    sout << config_tmp << "/" << slice3d->getName();
    featureDir = sout.str();
    mkdir(featureDir.c_str(), 0777);
  } else {
    featureDir = slice3d->inputDir;
  }
  
  stringstream sout_feature_filename;
  sout_feature_filename << featureDir << "/features_";
  ...
  bool reloadPrecomputedFeatures = false;
  if(config->getParameter("reloadPrecomputedFeatures", config_tmp)) {
    reloadPrecomputedFeatures = config_tmp[0] == '1';
  }
  
  if(reloadPrecomputedFeatures) {
...
void load_learn_parm(STRUCT_LEARN_PARM *sparm, Config* config)
{
  ...
  if(config->getParameter("use_symmetric_weights", config_tmp)) {
    sparm->useSymmetricWeights = atoi(config_tmp.c_str()) == 1;
  }
  SSVM_PRINT("[SVM_struct] useSymmetricWeights=%d\n", (int)sparm->useSymmetricWeights);
			       ...
			       STRUCT_LEARN_PARM *sparm)
{
  ...
  SSVM_PRINT("[finalize_iteration] ceps=%g,totalLossMVC=%g,n=%d,rhs=totalLossMVC/n=%g,totalLhsXw=%g,totalLhsXw/n=%g,slack=%g,totalWPsi=%g,totalWPsiGT=%g,C=%g\n",
             ceps, totalLossMVC,sample.n,rhs,totalLhsXw,lhsXw,slack,totalWPsi,totalWPsiGT,sparm->C);
